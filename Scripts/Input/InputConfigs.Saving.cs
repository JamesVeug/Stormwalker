...
public static partial class InputConfigs
{
    ...
    private static void ClientPrefsService_Reset()
    {
        ...
        {
            Plugin.Log($"Resetting input config for {map.name}");
    ...
    private static void ClientPrefsService_SaveInputConfig()
    {
        ...
        foreach (KeyValuePair<string, InputActionMap> pair in modNameToActionMaps)
        {
            ...
            Plugin.Log($"Saving input config for {pair.Key} with value {value}");
    ...
    private static void ClientPrefsService_LoadInputConfig()
    {
        ...
        if (!File.Exists(path))
        {
            Plugin.Log("No saved inputs found.");
            return;
        }
        ...
        if (savedInputs == null)
        {
            Plugin.Log("Couldn't load custom input json.");
            return;
        }
